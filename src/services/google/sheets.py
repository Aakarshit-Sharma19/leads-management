...
class SheetsService:
    ...
    def read_student_info_from_row(self, spreadsheet_id: str, row_index: int):
        ...
        try:
            ...
            raise service_exceptions.InvalidDataException(messages=f'The data present in the file is invalid.'
                                                                   f' Error: {e.errors()[0]["msg"]}.'
                                                                   f' Please notify {self._user.get_full_name()} to check'
                                                                   f' row number: {row_index}') from e

    def get_next_populated_row(self, spreadsheet_id: int, start_index: int, end_index: int):
    ...
    def set_row_from_student_response(self, spreadsheet_id: int, row_index: int, student_response: StudentResponse):
        row_range = f"A{row_index}:D{row_index}"
        try:
            execute_query(self._service.spreadsheets().values().update(spreadsheetId=spreadsheet_id, range=row_range,
                                                                       valueInputOption='USER_ENTERED',
                                                                       body=[student_response.to_tuple()]))
        except ValidationError as e:
            raise service_exceptions.InvalidDataException(
